...
def set_directories(dir_name, args = None, func = None, env_var = None):
    ...
    if args:
      ...
      hparams_dir = "%s|d%s_r%s_t%s_m%s_sigma%s"%(hparams_dir, env_var[0], env_var[1],
                                                    env_var[2], env_var[3], env_var[4])
...
def algos_vs_var_metrics(
  ...
  for metric in metrics:
    for algo in algos_dict:
      ...
      if list_var != 'k':
        if list_var == 't' and rho is not None:
            plot_func(rho, np.mean(metrics_list[algo][metric], axis=1),
                      algo_style, label=algo_label)  
            y = np.mean(metrics_list[algo][metric], axis = 1)
            ci = np.std(metrics_list[algo][metric], axis = 1)
            plt.fill_between(rho, (y-ci), (y+ci), color = algo_style[3], alpha=.1)
        else:
            plot_func(var_list, np.mean(metrics_list[algo][metric], axis=1),
                      algo_style, label=algo_label)
            y = np.mean(metrics_list[algo][metric], axis = 1)
            ci = np.std(metrics_list[algo][metric], axis = 1)
            plt.fill_between(var_list, (y-ci), (y+ci), color = algo_style[3], alpha=.1)
      else:
