...
def search(run_logs, param_dict):
    for i in range(run_logs.shape[0]):
        if run_logs[i, :-1] == np.array([d, r, t, m, sigma, algo]):
            return run_logs[i, -1]
    return None
    

parser = argparse.ArgumentParser(description='Visualization')
parser.add_argument('-d', type = int, nargs = "*", help = 'dimension')
parser.add_argument('-t', type = int, nargs = "*", help = 'num of tasks')
parser.add_argument('-m', type = int, nargs = "*", help = 'num of observations per task')
parser.add_argument('-s', type = float, nargs = "*", help = 'noise')
parser.add_argument('--alg', type = str, nargs = "*", default=['mom', 'altmingd_wd_lyr4', 'altmingd'], help = 'algorithm')
parser.add_argument('--metrics', type = str, nargs = "*", default=['avg_mse_loss', 'surrogate_loss'], help = 'metrics')
parser.add_argument('--loglog', type= str, default="None")
...
run_logs = np.loadtxt('./run_logs.csv', delimiter = ',', detype = 'str')
...
for algo in args.alg:
    for var in var_list:
        if list_var == 'd':
            ...
            homogeneity = var
        path = search(run_logs, {'d': d, 'r': r, 't': t, 'm': m, 'sigma': sigma, 'algo': algo})
        if path is None:
            raise ImportError
        f = open(path, 'rb')
        ...
        for metric in args.metrics:
            metrics_list[var][algo][metric].append(output['metrics_list'][var][algo][metric])
